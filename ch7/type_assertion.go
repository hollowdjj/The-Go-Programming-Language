...
T为接口类型时：x.(T)检查x的动态类型是否满足T。如果检查成功，x的动态值不会被提取，返回值是一个T类型的接口值。
...
func test() {
	var w io.Writer
	//注意os.Stdout是os包中一个类型为*os.Fileos的包一级变量，而*os.Fileos类型实现了 Write(p []byte) (n int, err error)方法
	w = os.Stdout
	...
	//w的动态类型为*os.File，有Write，Read方法，故满足io.ReadWriter，断言成功，返回io.ReadWriter类型的接口值。
	rw := w.(io.ReadWriter)
	fmt.Println(rw)
